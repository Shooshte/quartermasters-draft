...
writeFileSync(new URL('../../fixtures/engine-parity/battles.jsonl',import.meta.url),cases.map(c=>JSON.stringify({...c,expected:trace(c.input,c.options)})).join('\n')+'\n');
const seeds=[0,42,-1,9007199254740991,' 42 ','⚔️battle🛡️','\uFEFF seed \uFEFF',1e21,1e-7];
writeFileSync(new URL('../../fixtures/engine-parity/rng.json',import.meta.url),JSON.stringify(seeds.map(seed=>{const rng=createSeededRandom(seed);return {seed,values:Array.from({length:20},rng)}}))+'\n');
