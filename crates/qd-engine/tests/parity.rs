...
fn compare(actual: &Value, expected: &Value, path: &str) {
    match (actual, expected) {
        (Value::Number(a), Value::Number(b)) => {
            assert_eq!(a.as_f64(), b.as_f64(), "number at {path}")
        }
...
fn reference_battles_match_every_batch_and_log_entry() {
    for line in include_str!("../../../fixtures/engine-parity/battles.jsonl")
        .lines()
        .chain(include_str!("../../../fixtures/engine-parity/reference-tests.jsonl").lines())
    {
        ...
        for (i, batch) in case["expected"]["batches"]
            .as_array()
            .unwrap()
            .iter()
            .enumerate()
        {
            ...
            let new_logs = json!(logs[log_len..]);
            log_len = logs.len();
            actual["log"] = new_logs;
            compare(&actual, batch, &format!("{name}/batch-{i}"));
        }
        if engine.get_state()["status"] != "finished" {
            continue;
        }
...
fn utf16_seed_hash_and_mulberry32_match_typescript() {
    let cases: Vec<Value> =
        serde_json::from_str(include_str!("../../../fixtures/engine-parity/rng.json")).unwrap();
    for case in cases {
        compare(
            &json!(random_sequence(&case["seed"], 20).unwrap()),
            &case["values"],
