...
- **`apps/web`** — TanStack Start (v1.167.x) SSR app with React 19, Tailwind v4, shadcn/ui. Uses `vite.config.ts` with `tanstackStart()` plugin — no `app.config.ts` or entry files.
...
Internal packages export built `dist` artifacts, not source `.ts` files. Keep `build`, `typecheck`, and `dev` scripts in package manifests aligned with this contract.
...
- **Lint/typecheck**: `pnpm lint` runs Biome plus TypeScript checks. `pnpm typecheck` runs TypeScript only.
