...
describe("Table", () => {
  it("renders the container wrapper and slot metadata", () => {
    ...
    expect(screen.getByRole("columnheader", { name: "Unit" })).toHaveAttribute("data-slot", "table-head");
    expect(screen.getByRole("cell", { name: "Barbarian" })).toHaveAttribute("data-slot", "table-cell");
  });
  ...
  it("omits hover styles when disableHover is true", () => {
    ...
    expect(screen.getByRole("row", { name: "No hover" }).className).not.toContain("hover:bg-muted/50");
